...
from octis.models.sw_variants import RPSW, EBRPSW, DSW, SW
# WAE model
class WAE(nn.Module):
    ...
    def nstsw_loss(self, z, prior, p=2, delta=1, ntrees=100, nlines=50, noisy_mode=None, lambda_=0.0, p_noise=2, p_agg=2):
        dn = z.shape[-1]
        nstsw = NTWConcurrentLines(p=p, delta=delta, noisy_mode=noisy_mode, lambda_=lambda_, p_noise=p_noise, p_agg=p_agg)
        theta, intercept = ntw_generate_trees_frames(ntrees, nlines, dn)
        return nstsw(z, prior, theta, intercept)
