...
def nsts_unif(X, ntrees=250, nlines=4, p=2, delta=2, device='cuda', type='normal', noisy_mode=None, lambda_=0.0, p_noise=2):
    Y_unif = unif_hypersphere(X.shape, device=X.device) 
    stswd_unif = nsts(X, Y_unif, ntrees, nlines, p, delta, device, type, noisy_mode=noisy_mode, lambda_=lambda_, p_noise=p_noise)
    return stswd_unif